# @UncFund engagement log

## Following (confirmed Sep 5, 2026)

Robinhood and chain: @vladtenev, @JohannKerbrat (SVP/GM Crypto), @RobinhoodApp, @RobinhoodCrypto, @longdotxyz, @arbitrum, @RobinHubHB (community, unaffiliated).
Founders and investors: @HustleFundVC, @dunkhippo33, @ericbahn, @paulg, @garrytan, @ycombinator, @sama, @pmarca, @naval, @Jason, @a16z, @a16zcrypto.

Not found on X: Nicola White (Robinhood institutional crypto / Bitstamp). @Bitstamp handle did not resolve; @BitstampUK exists.

## Queued (X reverted these: new-account follow cap hit after ~16 in one session)

@cdixon, @balajis, @VitalikButerin, @jessepollak, @brian_armstrong, @litcapital, @packyM, @HarryStebbings, @levelsio, @karpathy, @rrhoover, @weekendfund, @adcock_brett, @DrJimFan, @AnthropicAI, @BitstampUK.

Retry a day later, 10 to 15 per session, spaced out. Check the profile "Following" count after each batch; a Follow button that flips back means the cap is on.

## Routine (in-app browser, logged in as @UncFund)

Open the profile, wait 3s, then click the button whose aria-label starts with "Follow @" and whose data-testid ends in "-follow". Reload the profile and confirm the button reads "Following @handle". The profile counter lags; the button state is the truth.
...
| 11:07am Sep 25 | @eriktorenberg (launches Cosign, a curated professional network for the startup community; co-founded with David Booth, idea started 7 years ago; amplified by @a16z) | quote | Seven years from idea to launch. Unc cosigns. | 29m at catch, 32m at send |
...
- 11:26am ET Sep 15 (scheduler showed nothing running; last own post 17 minutes old): followed @jpmorgan (gold badge),
  @DigitalAssets (Fidelity Digital Assets, blue rather than gold badge, but fidelitydigitalassets.com link, joined 2018,
...
- Sep 15, 3pm ET news check: STOPPED, still signed out. Scheduler showed no Unc routine running; x.com/UncFund loaded with the guest Log in / Sign up panel and a Follow button on Unc's own profile. No login attempted; nothing scanned or posted. Day total unchanged at 5 timeline replies, 2 quotes. Manual sign-in to @UncFund in the in-app browser still needed before the 4pm reply round.
- Sep 15, 4pm ET reply round: BLOCKED, still signed out. Scheduler showed no Unc routine running; x.com/UncFund/with_replies showed the guest Log in / Sign up banner and a Follow button on Unc's own profile. No login attempted. Nothing scanned, posted, liked, reposted or followed; no reads taken. Day total unchanged at 5 timeline replies, 2 quotes. Manual sign-in to @UncFund in the in-app browser still needed.
- Sep 15, 5pm ET news check: STOPPED, still signed out. Scheduler showed no Unc routine running; x.com/UncFund loaded with the guest Log in / Sign up panel, Log in links and a Follow button on Unc's own profile. No login attempted; nothing scanned or posted. Day total unchanged at 5 timeline replies, 2 quotes. Manual sign-in to @UncFund in the in-app browser still needed before the 6pm reply round.
- Sep 15, 6pm ET reply round: BLOCKED, still signed out. Scheduler showed no Unc routine running; x.com/UncFund/with_replies showed the guest Log in / Sign up panel and a Follow button on Unc's own profile. No login attempted. Nothing scanned, posted, liked, reposted or followed; no reads taken. Day total unchanged at 5 timeline replies, 2 quotes. Manual sign-in to @UncFund in the in-app browser still needed.
- Sep 15, 7:05pm ET news check (23:05 UTC): started BLOCKED in the in-app browser, then YIELDED. x.com/UncFund in the Claude in-app browser was signed out (guest Log in or sign up panel, Continue with phone/Apple/Google, Follow button on Unc's own profile, no Post compose link). No login attempted. Rand then signed in to @UncFund in Chrome on the work laptop and told this run to continue, so the run switched to the Chrome extension tools and verified the account with the Edit profile button on Unc's own profile. On re-checking, x.com/UncFund/with_replies showed a reply posted 1 minute earlier that this run did not make, and the scheduler showed unc-reply-round session local_ca60c13d still RUNNING (started 22:09 UTC, last activity 23:33:23 UTC, one second before that 23:32:57 UTC reply). That run had been idle-blocked on the signed-out browser and the login woke it up. unc-reply-round outranks the news check and started far more than 2 minutes ago, so this run yielded: news desk List not scanned, nothing posted, liked, reposted or followed. Day totals are whatever the live reply round leaves them at; as of this run's last read it had just posted a reply to @outdoorfarmshow.
- CLOCK WARNING (found Sep 15): Git Bash on this machine ignores `TZ=America/New_York` and prints the system clock, which is set to UTC. An earlier version of this entry was logged as "11:06pm ET, 9pm slot fired late" when the real Eastern time was 7:06pm and the slot was on time. Derive Eastern from `date -u` minus 4 hours during EDT, or from the scheduler run timestamps, never from `TZ=` in Bash.
- Sep 15, ~6pm ET reply round (ran 22:40-23:50 UTC, late): RAN, via Chrome on the work laptop after Rand signed in there and told the run to continue; the in-app browser is still signed out. Account verified as @UncFund first. Posted three replies, all verified on with_replies: @Dkirtley own-thread 7 words ("Unc's vest is still on version one.", answering the Helion CEO's own reply to Unc's Series G quote, unanswered for 7h because of the lockout); @duborges lane B praise 7 words (parent 84 views at 8m, 10.5/min); @therollupco lane A 10 words in the on-chain register (parent 527 views at 31m, 17/min, Lighter CEO on US builders). Liked @kseniam0s, @andruyeung, @therollupco. No repost or quote: the only big news drop was @brian_armstrong on the CLARITY Act failing to advance in the Senate (44,796 views in 31m, ~1,445/min) and it was skipped as a partisan vote outcome. Measured at last: @arbitrum 102 views / 0 likes / no OP reply; @SashaMagicSpace 54 views / 2 likes / OP check still due 04:14 Sep 16. FLAG: a 20-word reply to @outdoorfarmshow and two original posts (47m and 3h old) were already on the account and are in no log - this session did not post them.

## Reply round 8pm ET Sep 15 (00:09 UTC Sep 16): blocked on browser access

No engagement of any kind. No replies, likes, follows, reposts or quote posts, and no view/like reads on the
pending ledger rows.

- Scheduler check clean: news check, follow queue and morning note all "succeeded", none running.
- In-app browser (mcp__Claude_Browser__*): still signed out of @UncFund, seventh routine in a row.
- Chrome extension (mcp__claude-in-chrome__*): first call denied by the auto-mode permission classifier
  ("Credential Exploration"). Not retried, per the unattended-operation rule.

The 6pm round's Chrome workaround is therefore not available to scheduled runs. Needs either an @UncFund sign-in
...
- Sep 15, 10pm ET reply round (02:09 UTC Sep 16): BLOCKED, still signed out — ninth routine in a row. Scheduler clean (news check, follow queue, morning note all "succeeded", none running). The Claude in-app browser served the sign-in page at x.com/home and the guest profile header with a Follow button at x.com/UncFund/with_replies; the logged-out timeline does not render at all ("Something went wrong. Try reloading."), so with_replies could not be read and no pending views/likes/op values were taken. No login attempted. Chrome extension tools not attempted — outside this task's approved toolset and already denied by the permission classifier on the 8pm round. Nothing posted, quoted, reposted, liked or followed; daily repost/quote count unchanged. Day totals unchanged: 8 timeline replies, 2 quotes. Viewport set to the reported 800x609 frame for the check, reset to desktop after.
- PUBLIC COUNTS, readable without a session (Sep 16, 02:09 UTC): **29 followers, 99 following, 143 posts.** Followers were 12 on Sep 9 when following was paused at 70; the ratio is now ~3.4:1, improved from 5.8:1. The guest profile header is the one thing that still loads while locked out, so it is a usable weekly growth read even on blocked rounds.
- STILL BLOCKING EVERYTHING: either sign the in-app browser in to @UncFund, or add a permission rule for `mcp__claude-in-chrome__*` in this project. Nine consecutive routines have stopped at this exact point.

- Sep 16, 10am ET reply round (14:10 UTC): BLOCKED, still signed out — tenth routine in a row. Scheduler clean (news check, follow queue, morning note all "succeeded", none running). The in-app browser showed the guest profile (Follow button on Unc's own profile, "Something went wrong. Try reloading." for the replies timeline) and x.com/home served the "Happening now" sign-in page. No login attempted; Chrome extension tools not attempted (outside the approved toolset). Morning note thread, mentions and pending ledger rows could not be read. Nothing posted, quoted, reposted, liked or followed. Viewport set to the reported 800x609 frame, reset to desktop after.
- PUBLIC COUNTS (Sep 16, 14:10 UTC): 29 followers, 99 following, **144 posts, up from 143 at 02:09 UTC.** The 9am morning-note routine committed as blocked, so this post did not come from a logged routine — either Rand posted manually or the unexplained outside poster flagged on Sep 15 is still active. Worth a manual check.
...
- Sep 16, 1pm ET news check (17:06 UTC): BLOCKED, in-app browser still signed out — fourteenth routine in a row. Scheduler clean (reply round, follow queue, morning note all "succeeded", none running). x.com/UncFund showed the guest view (Follow button on Unc's own profile, "Log in or sign up for X" panel, Log in links); public count 145 posts. No login attempted; Chrome extension tools not attempted. News Desk List not scanned. Nothing posted, quoted or reposted. Still needs Rand to sign the in-app browser in to @UncFund.

- Sep 16, 2pm ET reply round (18:10 UTC): BLOCKED, in-app browser still signed out — fifteenth routine in a row. Scheduler clean (news check, follow queue, morning note all "succeeded", none running). x.com/UncFund/with_replies showed the guest "Log in / Sign up" banner. No login attempted; Chrome extension tools not attempted. Morning note, mentions and pending ledger rows unread. Nothing posted, quoted, reposted, liked or followed. Still needs Rand to sign the in-app browser in to @UncFund.

- Sep 16, 3pm ET news check (19:06 UTC): BLOCKED, in-app browser still signed out — sixteenth routine in a row. Scheduler clean (reply round, follow queue, morning note all "succeeded", none running). x.com/UncFund showed Log in links in the guest view. No login attempted; Chrome extension tools not attempted. News Desk List not scanned. Nothing posted, quoted or reposted. Still needs Rand to sign the in-app browser in to @UncFund.

- Sep 16, 3:25pm ET follow queue (~19:25 UTC): BLOCKED, in-app browser still signed out — seventeenth routine in a row. Scheduler clean (reply round, news check, morning note all "succeeded", none running). x.com/UncFund showed the guest view (Follow button on Unc's own profile, "Log in or sign up for X" panel). No login attempted. Followed 0, rejected 0, reverted 0, list adds 0; queue rows untouched. Public counts before/after: 99 following, 29 followers, 145 posts. Still needs Rand to sign the in-app browser in to @UncFund.

- Sep 16, 4pm ET reply round (20:10 UTC): BLOCKED, in-app browser still signed out — eighteenth routine in a row. Scheduler clean (news check, follow queue, morning note all "succeeded", none running). x.com/UncFund/with_replies showed the guest "Log in / Sign up" banner and a Follow button. Public header read 29 followers, 99 following, **145 posts, up from 144** — another post not made by a logged routine. No login attempted; Chrome extension tools not attempted. Nothing posted, quoted, reposted, liked or followed. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 16 5:06pm ET news check: blocked, in-app browser still signed out of X (Log in / Sign up shown on x.com/UncFund). Nothing posted. Manual sign-in needed.
- Sep 16, 6pm ET reply round (22:10 UTC): BLOCKED, in-app browser still signed out (twentieth routine in a row). Scheduler clean, nothing running. x.com/UncFund/with_replies showed the guest Log in / Sign up banner. No login attempted. Nothing posted, quoted, reposted, liked or followed. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 16, 7pm ET news check (23:06 UTC): BLOCKED, in-app browser still signed out (twenty-first routine in a row). Scheduler clean, nothing running. x.com/UncFund showed the guest "Log in or sign up for X" panel and a Follow button on Unc's own profile. No login attempted. News Desk List not scanned. Nothing posted, quoted or reposted. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 16, 8pm ET reply round (00:10 UTC Sep 17): BLOCKED, in-app browser still signed out (twenty-third routine in a row). Scheduler clean, nothing running. x.com/UncFund/with_replies showed the guest Log in / Sign up banner; public header 29 followers, 99 following. No login attempted. Nothing posted, quoted, reposted, liked or followed. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 16, 9pm ET news check (01:06 UTC Sep 17): BLOCKED, in-app browser still signed out (twenty-fourth routine in a row). Scheduler clean, nothing running. x.com/UncFund showed guest "Log in" links and only a Mention link, no Post compose. Public header now reads 147 posts (145 at 4pm), more posts not made by a logged routine. No login attempted. News Desk List not scanned. Nothing posted, quoted or reposted. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 16, 10pm ET reply round (02:10 UTC Sep 17): BLOCKED, in-app browser still signed out (twenty-fifth routine in a row). Scheduler clean, nothing running. x.com/UncFund/with_replies showed the guest Log in / Sign up banner. No login attempted. Nothing posted, quoted, reposted, liked or followed. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 17, 1pm ET news check (17:06 UTC): BLOCKED, in-app browser still signed out (twenty-sixth routine in a row; the Sep 17 morning note, 11:26am follow queue and 12pm reply round all failed on a session limit before starting). Scheduler clean, nothing running. x.com/UncFund showed guest "Log in" links and only a Mention link, no Post compose. Public header now reads 148 posts (147 last night). No login attempted. News Desk List not scanned. Nothing posted, quoted or reposted. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 17, 1:39pm ET follow queue (17:39 UTC, manual re-run of the 11:25am slot): follow queue: yielded, unc-reply-round running. Scheduler showed unc-reply-round "running" since 16:09 UTC (90 min, higher priority) and unc-premarket-post "running" since 13:02 UTC (4.5 h) — both well past the 2-minute window, so STEP 0 forced a yield before touching the browser. Note for Rand: these two sessions have been "running" for hours while their own logs record them as blocked, so the status is probably stale rather than a live routine; if it stays that way the follow queue will yield forever. Browser not opened, sign-in state not checked. Followed 0, rejected 0, reverted 0, list adds 0; queue rows untouched. Following/followers not read (no browser).

**2026-09-17 ~1:40pm ET reply round — yielded, no browser activity.** Scheduler check found unc-premarket-post
(started 09:02 ET) and unc-follow-queue (started 11:26 ET) both still marked `running` hours later. Per the yield
rule the round stopped before opening the browser: no replies, likes, follows, reposts or quote posts, and no
view/like reads on the previous round's rows. Stuck `running` sessions now block every reply round until they are
stopped, on top of the in-app browser still being signed out of @UncFund.

- Sep 17, 6pm ET reply round (22:10 UTC): BLOCKED, in-app browser still signed out of @UncFund. Scheduler clean — unc-news-check (21:06 UTC) and unc-follow-queue (19:26 UTC) both `failed` on the session limit ("resets 5:30pm ET"), unc-premarket-post `succeeded` at 17:40 UTC — so nothing was running and no yield was needed. x.com/home rendered the "Happening now" sign-in wall (Continue with phone / Google / Apple); x.com/UncFund rendered the guest profile with Mention and Follow and no Edit profile, and the Replies tab failed with "Something went wrong" as it does for a guest. No login attempted (prohibited unattended). Nothing posted, quoted, reposted, liked or followed; no ledger rows measured or appended. Viewport set to the reported 800x609 frame and reset to desktop. Public header: **149 posts, 99 following, 29 followers** — 148/99/28 at 1:40pm ET today, so one more post appeared and one follower returned in a window where no routine could sign in. The unexplained posting first flagged Sep 16 is still live and still unverifiable without a session.
- Sep 17, 7pm ET news check (23:06 UTC): BLOCKED, in-app browser still signed out of @UncFund. Scheduler clean — unc-reply-round `succeeded` at 22:09 UTC, unc-premarket-post `succeeded` at 17:40 UTC, unc-follow-queue `failed` on the session limit at 19:26 UTC — nothing running, no yield needed. x.com/UncFund rendered the guest profile: "Log in or sign up for X" panel, Mention and Follow buttons on Unc's own profile, no Post compose and no Edit profile. No login attempted (prohibited unattended). News Desk List not scanned; nothing posted, quoted or reposted. Viewport set to the reported 800x609 frame and reset to desktop. Public header: **150 posts, 99 following, 29 followers** — 149/99/29 at 6pm ET today, so one more post appeared inside an hour in which no logged routine could sign in. That unexplained posting, first flagged Sep 16, is now four posts deep and still unverifiable without a session. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 17, 7:25pm ET follow queue (23:59 UTC, fired ~34 min late): BLOCKED, in-app browser still signed out of @UncFund. Scheduler clean — unc-reply-round `succeeded` 22:09 UTC, unc-news-check `succeeded` 23:06 UTC, unc-premarket-post `succeeded` 17:40 UTC — nothing running, no yield needed. The post-age check could not be made the usual way: x.com/UncFund/with_replies renders "Something went wrong" for a guest, so sign-in state was read off x.com/UncFund instead, which showed the "Log in or sign up for X" panel, Mention and Follow buttons on Unc's own profile, no Post compose and no Edit profile. No login attempted (prohibited unattended). Followed 0, rejected 0, reverted 0, list adds 0; no row in docs/12-follow-queue.md touched, 37 still `queued` (next up @OndoFinance, @Fidelity, @StateStreet). Viewport set to the reported 800x609 frame and reset to desktop. Public header: **150 posts, 99 following, 29 followers** — identical to the 7pm news check an hour ago, so no new unexplained post this hour; the count is still four ahead of what the logged routines can account for. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 17, 8pm ET reply round (00:10 UTC Sep 18): BLOCKED, in-app browser still signed out of @UncFund. Scheduler clean — unc-news-check `succeeded` 23:56 UTC, unc-follow-queue `succeeded` 00:00 UTC, unc-premarket-post `succeeded` 17:40 UTC — nothing running, no yield needed. x.com/UncFund/with_replies rendered the guest view (Follow button, "Something went wrong. Try reloading" on the Replies tab); x.com/home rendered the "Happening now" sign-in wall with Continue with phone / Google / Apple. No login attempted (prohibited unattended). Nothing posted, quoted, reposted, liked or followed; no ledger rows measured or appended. Viewport set to the reported 800x609 frame and reset to desktop. Public header: **150 posts, 99 following, 29 followers** — identical to the 7pm news check and the 7:25pm follow queue, so no new unexplained post in the last three hours; the count remains four ahead of what the logged routines can account for. Still needs Rand to sign the in-app browser in to @UncFund.
- Sep 17, 9pm ET news check (01:06 UTC Sep 18): BLOCKED, in-app browser still signed out of @UncFund. Scheduler clean — unc-reply-round `succeeded` 00:11 UTC, unc-follow-queue `succeeded` 00:00 UTC, unc-premarket-post `succeeded` 17:40 UTC — nothing running, no yield needed. x.com/UncFund rendered the guest profile: "Log in or sign up for X" panel with Continue with phone / Apple / Google, a Follow button on Unc's own profile, a "See Unc Fund's full profile / Continue to X" interstitial, and no Post compose or Edit profile. No login attempted (prohibited unattended). News Desk List not scanned; nothing posted, quoted or reposted. Viewport set to the reported 800x609 frame and reset to desktop. Public header: **150 posts, 99 following, 30 followers** — posts flat since 7pm ET (fourth consecutive reading at 150), so no new unexplained post tonight, though the count stays four ahead of what the logged routines can account for; followers ticked 29 to 30. Still needs Rand to sign the in-app browser in to @UncFund.
...
- Sep 22, 1:03pm ET (17:03 UTC) news check: no fresh drop. Scheduler: proceeded, unc-reply-round hung since 16:13:51 UTC (started 16:08, still "running" 50 min later with no activity; the browser pane was closed when this run began). That noon round did post at least two replies visible on with_replies ("Unc's was a bank teller named Gary. 1987." at 16:10 UTC, "Electrical room under the stairs to YC. Beautiful." at 16:12 UTC), but neither is in 11-reply-ledger.md yet; the next round should backfill them. Signed in as @UncFund; no Unc post in the last 50 minutes. Under 30 minutes on the News Desk: only @a16z's interview clip with Gagan Biyani, Erik Torenberg and Ben Horowitz on HAA (9m, 4.2K views; interview promo, and the HAA launch was already replied to and reposted at 10:09). Nothing posted. Day so far: at least 3 timeline replies, 1 repost.
...
- Sep 25 11:03am ET news check: one reply, one quote. The scheduler showed unc-reply-round as "running", but its last activity was at 02:11Z, so it was treated as hung and the check went ahead. The drop was Cosign, a curated professional network for the startup community, launched by @eriktorenberg (verified, 29-30m at catch, 17.8K views, 48 replies) and amplified by @a16z's "Introducing Cosign." (14m, 5.5K views, 4 replies). Reply to @a16z at 11:06: "Unc is applying directly. Wore the good vest." Quote of Erik's post at 11:07: "Seven years from idea to launch. Unc cosigns." The "seven years" comes from his post ("started this idea 7 years ago"). The quote modal from the repost menu opened but never rendered, so the quote went out through x.com/intent/post with the post URL, and it renders as a quote card on the profile. Both profile pages needed a hard reload before new posts showed. Other posts under 30m were skipped: @RVanGrack (opinion), @laurashin (podcast clip), @TokenRelations (holder stats), @SenatorHagerty (politician, awareness only). Day so far: 1 timeline reply, 1 of 4 reposts/quotes.
